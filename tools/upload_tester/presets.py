...
def _decode_preset(preset: dict, callback_url: Optional[str]) -> dict:
    ...
    if not b64:
        ...
        if not out["is_binary"] and callback_url is not None:
            text = raw.decode("utf-8", errors="surrogateescape")
            text = _CALLBACK_RE.sub(callback_url.rstrip("/"), text)
            raw = text.encode("utf-8", errors="surrogateescape")
        out["content"] = raw

    # Also replace {{CALLBACK}} in filename for OOB filename-injection presets
    if callback_url is not None and "{{CALLBACK}}" in out["filename"]:
        out["filename"] = out["filename"].replace(
